...
public class RegistrationPage extends TestBase{
	...
	public void Registration(String ssn11, String ssn22, String ssn33, String acntnmbr, String phID, String lname, String fname, 
			String stID, String payfrq, String netincome, String groceincome){
		...
		statedd.selectByVisibleText(stID);
		if (stID.equalsIgnoreCase("Missouri")) {
			postalID.sendKeys("63010");
		}		
		else if (stID.equalsIgnoreCase("Kansas")) {
			postalID.sendKeys("66103");
		}
		else if (stID.equalsIgnoreCase("Texas")) {
			postalID.sendKeys("75457");
		}
		else if (stID.equalsIgnoreCase("Nevada")) {
			postalID.sendKeys("89148");
		}
		else if (stID.equalsIgnoreCase("Illinois")) {
			postalID.sendKeys("62462");
		}
		else if (stID.equalsIgnoreCase("Idaho")) {
			postalID.sendKeys("83211");
		}
		else if (stID.equalsIgnoreCase("Nebraska")) {
			postalID.sendKeys("68106");
		}
		else if (stID.equalsIgnoreCase("xxxxx")) {
			postalID.sendKeys("");
		}
		...
		email_check.click();
		photoid.sendKeys(phID);
		Select IDStatedd=new Select(IDState_dd);
		IDStatedd.selectByVisibleText(stID);
		...
		net_income.sendKeys(netincome);
		gross_income.sendKeys(groceincome);
		Select payfreq=new Select(pay_freq);
		payfreq.selectByVisibleText(payfrq);
		if (payfrq.equalsIgnoreCase("Monthly")) {
			Select monthlyDatedd=new Select(monthlyDate);
			monthlyDatedd.selectByVisibleText("Last Day");	
		}
		else if (payfrq.equalsIgnoreCase("Semi-Monthly")){
			rad_semi.click();	
		}
		else if (payfrq.equalsIgnoreCase("Weekly")) {
			friday.click();
		}
		else {
			friday.click();
		...
		accountNbrDisp.sendKeys(acntnmbr);
		checkAccountNbrDisp.sendKeys(acntnmbr);
